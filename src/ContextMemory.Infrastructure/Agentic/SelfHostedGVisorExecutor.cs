...
public sealed class SelfHostedGVisorExecutor : IToolExecutor
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (resolved is null)
        {
            ...
            {
                Output = ToolExecutionMessages.ToolNotRegisteredSelfHosted(toolCall.Function.Name, runtimeConfig),
                ExitCode = 1
            };
        }

        var (execution, runtime) = resolved.Value;
        if (execution.SandboxEndpoint is not { Length: > 0 } endpoint)
        {
            return new ToolExecutionResult
            {
                Output = ToolExecutionMessages.SandboxEndpointNotConfigured(runtime, runtimeConfig),
                ExitCode = 1
            };
        }

        if (!AgenticNetworkEgressPolicy.IsSandboxEndpointAllowed(runtimeConfig, execution, endpoint))
        {
            _logger.LogWarning(
                "Sandbox egress blocked for tenant {AppId} endpoint {Endpoint}",
                appId,
                endpoint);
        ...
        return runtime switch
        {
            ...
            {
                Output = ToolExecutionMessages.UnsupportedSelfHostedRuntime(runtime, runtimeConfig),
                ExitCode = 1
            }
        };
    }
        ...
        AppRuntimeConfig runtimeConfig)
    {
        ...
        {
            if (!string.Equals(execution.Type, "self-hosted-sandbox", StringComparison.OrdinalIgnoreCase))
                continue;
